...
function BaseApp:zoom(increment)rtk.scale.user=rtk.clamp(rtk.scale.user+increment,0.5,4.0)log.info('zoom %.02f', rtk.scale.user)self:set_statusbar(string.format('Zoom UI to %.02fx', rtk.scale.user))self.config.scale=rtk.scale.user
self:save_config()end
...
function BaseApp:_set_window_pinned(pinned)if rtk.has_js_reascript_api then
...
self:zoom(1.0-rtk.scale.user)event:set_handled()end
end
...
if self.msblsb then
local hash=string.format('%016x', self:_hash(false))local hash2=string.format('%016x', crc64(hash))self.guid=string.format('11111111-%s-%s-%s-%s',hash2:sub(1,4),hash2:sub(5,8),hash:sub(1,4),hash:sub(5))else
...
function rfx.opcode_flush(track,fx,gmem_index,opcode_param)if rfx.rfx_awaiting_commit then
rfx._opcode_commit(track,fx,gmem_index)end
...
assert(self.banks_by_channel, 'Called sync_banks_to_rfx() before calling index_banks_by_channel_and_check_hash()')log.time_start()reaper.Undo_BeginBlock2(0)rfx.push_state(self.track)self:opcode(rfx.OPCODE_CLEAR)for channel=1,16 do
local banks=self.banks_by_channel[channel]
if banks then
for _,bank in ipairs(banks)do
bank:realize()local param1=(channel-1)|(0<<4)self:opcode(rfx.OPCODE_NEW_BANK,{param1,bank.msb,bank.lsb})for _,cc in ipairs(bank:get_chase_cc_list())do
...
local rtk=rtk
local log=rtk.log
local feedback={SYNC_CC=1,SYNC_ARTICULATIONS=2,SYNC_CHANNEL=4,SYNC_ALL=1|2|4,track=nil,track_guid=nil
...
function feedback.create_feedback_track()log.info('creating track for MIDI feedback')reaper.PreventUIRefresh(1)local idx=reaper.CountTracks(0)reaper.InsertTrackAtIndex(idx,false)feedback.track=reaper.GetTrack(0,idx)feedback.track_guid=reaper.GetTrackGUID(feedback.track)reaper.GetSetMediaTrackInfo_String(feedback.track, 'P_NAME', "MIDI Feedback (Reaticulate)", true)local fx=reaper.TrackFX_AddByName(feedback.track,BUS_TRANSLATOR_FX_NAME,0,1)reaper.TrackFX_Show(feedback.track,fx,2)feedback.update_feedback_track_settings()reaper.SetMediaTrackInfo_Value(feedback.track, 'B_SHOWINTCP', 0)reaper.SetMediaTrackInfo_Value(feedback.track, 'B_SHOWINMIXER', 0)feedback.scroll_mixer(app.track)reaper.PreventUIRefresh(-1)return feedback.track
end
...
function App:_insert_articulation(rfxtrack,bank,program,channel,take)local track=rfxtrack.track
...
local ppq=reaper.MIDI_GetPPQPosFromProjTime(take,cursor)insert_ppqs={{take,ppq,nil,program}}else
local item=reaper.CreateNewMIDIItemInProj(track,cursor,cursor+1,false)reaper.SetMediaItemInfo_Value(item, 'B_LOOPSRC', 0)take=reaper.GetActiveTake(item)local ppq=reaper.MIDI_GetPPQPosFromProjTime(take,cursor)insert_ppqs={{take,ppq,nil,program}}end
end
if delete_ppqs then
...
if recording then
local msb,lsb=bank:get_current_msb_lsb()reaper.StuffMIDIMessage(0,0xb0+srcchannel,0,msb)reaper.StuffMIDIMessage(0,0xb0+srcchannel,0x20,lsb)reaper.StuffMIDIMessage(0,0xc0+srcchannel,art.program,0)art.button.start_insert_animation()return
end
...
reaper.PreventUIRefresh(1)reaper.Undo_BeginBlock2(0)for i=0,reaper.CountSelectedTracks(0)-1 do
local track=reaper.GetSelectedTrack(0,i)local take=midi_track==track and midi_take
if track==rfx.current.track then
self:_insert_articulation(rfx.current,bank,art.program,srcchannel,take)else
local rfxtrack=rfx.Track()if rfxtrack:presync(track)then
...
local args=string.split(arg, ',')local channel=_cmd_arg_to_channel(args[1])local program=tonumber(args[2])local force_insert=tonumber(args[3] or 0)local art=nil
...
function App:handle_onupdate()BaseApp.handle_onupdate(self)self:check_sloppy_focus()local track=reaper.GetLastTouchedTrack()if track and not reaper.IsTrackSelected(track)then
...
self:check_banks_for_errors()end
self.screens.banklist.update()if self.screens.trackcfg.widget.visible then
self.screens.trackcfg.update()end
end
...
art.button:animate{attr='color', dst='red', duration=0.3, easing='in-out-sine'}:after(function()art.button:animate{attr='color', dst=color, duration=0.2, easing='linear'}end)end
...
local feedback=__mod_feedback
local log=rtk.log
local screen={minw=250,max_bankui_width=650,widget=nil,banklist=nil,track=nil,error=nil
